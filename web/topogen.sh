...
STATES_FILTER="['AUS', 'BIH', 'BRA', 'CAN', 'CHL', 'DNK', 'GRC', 'IND', 'MYS', 'RUS']"
...
("$NODE_MODULES_PATH/shp2json" -n build/US_Control_Areas.shp \
  | "$NODE_MODULES_PATH/ndjson-map" 'd.properties' \
...
# Parse countries but exclude continental USA.
...
("$NODE_MODULES_PATH/shp2json" -n build/${COUNTRIES_FILENAME}.shp \
  ...
  | "$NODE_MODULES_PATH/ndjson-filter" 'd.id && d.id != "USB"' \
