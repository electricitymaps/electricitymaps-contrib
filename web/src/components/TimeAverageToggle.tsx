    ...
    >
      {options.map(({ value, label, dataTestId }) => (
        <ToggleGroupItem
          ...
          className={`
          h-full grow basis-0 select-none text-xs font-semibold capitalize
            ${
              timeAverage === value
                ? 'rounded-full bg-white/80 text-brand-green outline outline-1 outline-neutral-200 dark:bg-gray-600/80 dark:text-white dark:outline-gray-400/10'
                : ''
            }`}
        >
          {label}
