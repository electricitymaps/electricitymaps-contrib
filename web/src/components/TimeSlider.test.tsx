...
describe('getThumbIcon', () => {
  it('returns "<MoveHorizontal size={16} />" when no index or sets are provided', () => {
    const { container: expected } = render(
      <MoveHorizontal size={16} pointerEvents="none" />
    );
  ...
  it.each([[10], [35]])(
    ...
    (index) => {
      ...
      const { container: expected } = render(<Moon size={16} pointerEvents="none" />);
  ...
  it.each([[5], [25], [50]])(
    'returns "<Sun size={16} />" when the index %i is not within a night time set',
    (index) => {
      const sets = [
        [7, 21],
        [30, 40],
      ];
      const { container: expected } = render(<Sun size={16} pointerEvents="none" />);
      const { container } = render(getThumbIcon(index, sets));
      expect(container.innerHTML).toEqual(expected.innerHTML);
    }
  );
