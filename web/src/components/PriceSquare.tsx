...
function PriceSquare({ price, tooltipContent }: PriceSquareProps) {
  const { t } = useTranslation();
  const priceScale = usePriceColorScale();
  const [{ backgroundColor }] = useSpring(
    {
      backgroundColor: priceScale(price),
    },
    [priceScale, price]
  );
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <TooltipWrapper tooltipContent={tooltipContent} side="bottom" sideOffset={8}>
        <div className="relative flex flex-col items-center">
          <div className="size-20 p-1">
            <animated.div
              style={{
                color: getTextColor(priceScale(price)),
                backgroundColor,
              }}
            ...
            >
              <p
                ...
                data-testid="price-square-value"
              >
                <span className="font-semibold">{Math.round(price) || '?'}</span>
                <span className="text-xs font-semibold">€</span>
              </p>
              <div className="text-xxs font-semibold leading-none">/MWh</div>
            </animated.div>
          </div>
      ...
      <p className="text-xs font-semibold text-neutral-600 dark:text-neutral-400">
        {t('country-panel.Price')}
      </p>
    </div>
  );
}

export default memo(PriceSquare);
