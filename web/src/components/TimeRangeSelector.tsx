  ...
  return (
    <DropdownMenu.Root onOpenChange={onToggleDropdown} open={isOpen} modal={false}>
      <DropdownMenu.Trigger>
        <div className="w-33 capitalize-first-letter flex flex-row items-center justify-between rounded-xl bg-white p-1 pl-2 text-sm font-semibold outline outline-1 outline-neutral-200 hover:bg-neutral-100 dark:bg-neutral-900 dark:outline-neutral-700 dark:hover:bg-neutral-800">
          {selectedLabel}
      ...
      >
        {options.map(({ value, label, dataTestId }) => (
          <DropdownMenu.Item
            ...
            className={`capitalize-first-letter h-full grow basis-0 select-none rounded-xl p-2 text-xs font-semibold hover:bg-neutral-100 focus-visible:outline-none dark:hover:bg-neutral-800 `}
          >
            {label}
          </DropdownMenu.Item>
        ))}
