    ...
    >
      ...
      >
        <div
          ...
          className="bg-sidebar flex h-full w-full flex-col border-r border-stroke group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-stroke group-data-[variant=floating]:shadow"
        >
          {children}
        </div>
      </div>
...
>(({ className, ...props }, reference) => (
  <div
    ref={reference}
    data-sidebar="header"
    className={twMerge('flex flex-col gap-2 px-2 pb-[0.90rem] pt-[1.15rem]', className)}
    {...props}
  />
));
...
const SidebarContent = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'>>(
  ({ className, ...props }, reference) => (
    <div
      ...
      className={twMerge(
        'flex min-h-0 flex-1 flex-col gap-2 overflow-auto pt-1 group-data-[collapsible=icon]:overflow-hidden',
