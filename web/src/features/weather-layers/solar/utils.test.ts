...
describe('splitRGBA', () => {
  it('works with rgb()', () => {
    ...
    expect(result).to.deep.eq([255, 0, 0, 1]);
  });
  it('works with rgba()', () => {
    const result = splitRGBA('rgb(255, 0, 0, 0.23)');
    expect(result).to.deep.eq([255, 0, 0, 0.23]);
  });
});
...
describe('solarColor', () => {
  it('works', () => {
    ...
    expect(result).to.deep.eq('rgba(0, 0, 0, 0.806)');
  });
});
...
describe('opacityToSolarIntensity', () => {
  it('works', () => {
    ...
    expect(result).to.deep.eq(380);
  });
});
describe('solarColorComponents', () => {
  it('works ', () => {
    const result = solarColorComponents[380];
    expect(result).to.deep.eq({ alpha: 52, blue: 0, green: 0, red: 0 });
  });
});
