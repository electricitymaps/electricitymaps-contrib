...
import zonesInfo from '../../../../../config/zones.json';
// TODO: Remove all unecessary id and class tags
const actionHasData = resolvePath('images/contrib-actions/action_hasdata.png');
const actionContact = resolvePath('images/contrib-actions/action_contact.png');
const actionHasNothing = resolvePath(
  'images/contrib-actions/action_hasnothing.png',
);

const HasNothing = () => (
  <div>
    <img className="contribute" src={actionHasNothing} alt="No data" />
    ...
    <h3>Add contact details of organisations that may have this data</h3>
    <p>
    ...
    <p>
      ...
      <br />
      <a href="https://github.com/tmrowco/electricitymap-contrib/wiki" target="_blank" rel="noopener noreferrer">Github Wiki </a>
    </p>
  </div>
);
...
const HasData = ({ githubIssues }) => (
  <div>
    <img className="contribute" src={actionHasData} alt="Parser" />
...
const HasContacts = ({ contacts }) => (
  <div>
    <img className="contribute" src={actionContact} alt="Contact" />
    <h2>No data is currently available for this region</h2>
    <p>electricityMap is powered by our open-source community</p>
    <p>
      Currently no data source has been identified that could allow us to put
    ...
    {contacts.map(contact => (
      <div className="contact-item">
        ...
        {contact.twitter && <p>Twitter: <a href={contact.twitter} target="_blank" rel="noopener noreferrer">{contact.twitter}</a></p>}
      </div>
    ))}
    <h3>Add a data source</h3>
    <p>
      If you know where we can find real-time data about electricity in this
      country, you can help us add data on the map by sharing the data source on
      Github <a href="https://github.com/tmrowco/electricitymap-contrib/issues" target="_blank" rel="noopener noreferrer">here.</a>
    </p>
  </div>
);
    ...
    <React.Fragment>
      ...
      <ul>
        {(githubIssues || []).map(issue => (
          <li key={issue.title}>
            <a key={issue.title} target="_blank" rel="noopener noreferrer" href={issue.url}>
              {issue.title}
            </a>
          </li>
        ))}
