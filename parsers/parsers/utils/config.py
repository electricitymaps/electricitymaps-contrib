...
CONFIG_DIR = Path(__file__).parent.parent.parent.parent.resolve() / 'config'

# Prepare zone bounding boxes
ZONE_BOUNDING_BOXES = {}

# Read parser import list from config jsons
ZONES_CONFIG = json.load(open(CONFIG_DIR / 'zones.json'))
EXCHANGES_CONFIG = json.load(open(CONFIG_DIR / 'exchanges.json'))
...
CO2EQ_PARAMETERS = json.load(open(CONFIG_DIR / 'co2eq_parameters.json'))
