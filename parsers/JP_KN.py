...
def fetch_production(zone_key='JP-KN', session=None,
    ...
    """
    This method adds nuclear production on top of the solar data returned
    by the JP parser. It tries to match the solar data with the nuclear 
    data. If there is a difference of more than 30 minutes between solar 
    and nuclear data, the method will fail.
    """
...
def extractCapacity(tr):
    ...
    and it uses the chinese symbol for 10k(万). If 
    this changes, the method will become inaccurate.
    """
...
def extractTime(soup):
    """
    Time is located in an image. It tries to decipher
    the text containing the data and assumes there will
    only be 4 digits making up the datetime.
    """
...
def get_nuclear_production():
    """
    Fetches all the rows that contains data of nuclear
    units and calculates the total kw generated by all plants.
    Illogically, all the rows has the class "mihama_realtime"
    which they might fix in the future.
    """
