...
def fetch_production(zone_key="CA-AB", session=None, target_datetime=None, logger=None):
    ...
    df_generations = pd.read_html(
        response.text, match="GENERATION", skiprows=1, index_col=0, header=0
    )
...
def fetch_price(zone_key="CA-AB", session=None, target_datetime=None, logger=None):
    ...
    for rowIndex, row in prices.iterrows():
        ...
        if isfloat(price):
            ...
            data[rowIndex] = {
                "datetime": arrow.get(rowIndex, "MM/DD/YYYY")
                .replace(hour=hour, tzinfo=ab_timezone)
                .datetime,
                "zoneKey": zone_key,
                "currency": "CAD",
                "source": "ets.aeso.ca",
