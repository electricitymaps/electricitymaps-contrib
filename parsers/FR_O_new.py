...
print(PRODUCTION_MAPPING)
print(STORAGE_MAPPING)


def generate_url(zone_key, target_datetime):
    return f"{DOMAIN_MAPPING[zone_key]}/api/v2/catalog/datasets/{HISTORICAL_DATASETS[zone_key] if target_datetime else LIVE_DATASETS[zone_key]}/records"


def fetch_data(
    zone_key="", session=None, target_datetime=None, logger=None, validated=False
) -> List[Dict[str, Any]]:
    ...
    if target_datetime and zone_key in HISTORICAL_DATASETS.keys():
        target_datetime_string: Optional[str] = target_datetime.strftime("%Y-%m-%d")
    elif target_datetime is None and zone_key not in HISTORICAL_DATASETS.keys():
    ...
    URL_QUERIES: Dict[str, Union[str, None]] = {
        "refine": "statut:Validé" if validated else None,
        "limit": "12" if target_datetime else "72",
        "timezone": "UTC",
        "order_by": f"{DATE_STRING_MAPPING[zone_key]} desc",
        "refine": f"{DATE_STRING_MAPPING[zone_key]}:{target_datetime_string}"
        if target_datetime
        else None,
    }
    ...
    for record in data["records"]:
        ...
        for key in record["record"]["fields"]:
            if key in PRODUCTION_MAPPING:
                ...
                storage[STORAGE_MAPPING[key]] = (
                    record["record"]["fields"][key]
                    if STORAGE_MAPPING[key] not in storage.keys()
                    else storage[STORAGE_MAPPING[key]] + record["record"]["fields"][key]
                )
        return_list.append(
            {
                "zoneKey": zone_key,
                "datetime": datetime.fromisoformat(
                    record["record"]["fields"][f"{DATE_STRING_MAPPING[zone_key]}"]
                ),
...
@refetch_frequency(timedelta(hours=12))
def fetch_production(
    zone_key="FR-COR", session=None, target_datetime=None, logger=None
):
    return fetch_data(zone_key, session, target_datetime, logger)


@refetch_frequency(timedelta(hours=12))
def fetch_estimated_production(
    zone_key="FR-COR", session=None, target_datetime=None, logger=None
):
    return fetch_data(zone_key, session, target_datetime, logger, validated=False)
