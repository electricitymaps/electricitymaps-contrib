...
def fetch_production(
    ...
    for event in island_data:
        ...
        storage = StorageMix()
        # Due to the way the response object from REE works we can't use the set_value function.
        # Instead we have to manually set the values for each key.
        # TODO: Change this upstream in the REE library.
        production.biomass = event.waste
        production.coal = event.carbon
        production.gas = round((event.gas + event.combined), 3)
        production.hydro = event.hydraulic
        production.oil = round((event.vapor + event.diesel), 3)
        production.solar = event.solar
        production.unknown = event.other
        production.wind = event.wind
        ### Zone overrides
        ...
        if zone_key.split("-")[1] == "CN":
            production.gas = None
            production.oil = round(
                (event.vapor + event.diesel + event.gas + event.combined),
                3,
            )
            # Hydro response is hydro storage
            if zone_key == "ES-CN-HI":
                production.hydro = None
                storage.hydro = -event.hydraulic
