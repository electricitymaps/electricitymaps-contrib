...
def fetch_production(
    ...
    for event in island_data:
        ...
        for mode in [
            attribute
            for attribute in getmembers(event)
            if not attribute[0].startswith("__")
        ]:
            if mode[0] in PRODUCTION_MAPPING.keys():
                ...
                production.add_value(mode=PRODUCTION_MAPPING[mode[0]], value=mode[1])

                # Zone specific override for El Hierro
                # Hydro response is hydro storage, not hydro production
                if zone_key == "ES-CN-HI" and mode[0] == "hydraulic":
                    storage.hydro = -mode[1]
                    production.hydro = 0
