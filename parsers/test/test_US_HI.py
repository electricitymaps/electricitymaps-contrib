...
class TestUSHI(unittest.TestCase):
    def setUp(self):
        ...
        self.adapter.register_uri('GET', US_HI.BASE_URL + 'limit=1', content=realtime_response_data)

        hist_response_data = resource_string('parsers.test.mocks', 'US_HI_hist.htm')
        hist_dt = arrow.get('2013/12/04 16:30').format('YYYY-MM-DD')
        self.adapter.register_uri('GET', US_HI.BASE_URL + f"date={hist_dt}", content=hist_response_data)
    ...
    def test_fetch_production_historical(self):
        ...
        data = US_HI.fetch_production(session=self.session, logger=self.logger, target_datetime=test_target_dt)
        self.assertIsInstance(data, dict)
        self.assertEqual(data['zoneKey'], 'US-HI-OA')
    ...
    def test_fetch_production_outdated(self):
        ...
        old_data = US_HI.fetch_production(session=self.session, logger=self.logger, target_datetime=test_target_dt)
