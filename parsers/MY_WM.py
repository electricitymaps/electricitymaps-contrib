...
def data_processer(rawdata, logger) -> tuple:
    ...
    mapped_generation = [
        (fuel_mapping.get(gen_type, "unknown"), val)
        for gen_type, val in current_generation.items()
    ]
    ...
    for gen_type in unmapped:
        logger.warning(
            "{} is missing from the MY generation type mapping!".format(gen_type)
        )
...
def post_to_switch(tie, session):
    ...
    switch_tie = {
        "__VIEWSTATE": hidden_values["viewstate"],
...
def zip_and_merge(egat_data, hvdc_data, logger) -> list:
    ...
    if mismatch_count > 0:
        logger.warning(
            "{} date mismatches between EGAT and HVDC ties found for MY-WM->TH, skipped datapoints.".format(
                mismatch_count
            ),
            extra={"key": "MY-WM"},
        )
...
def fetch_exchange(
    ...
    if sortedcodes == "MY-WM->TH":
        ...
        raise NotImplementedError(
            "The exchange {} is not implemented".format(sortedcodes)
        )
