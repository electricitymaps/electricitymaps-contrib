...
def fetch_production(
    ...
    if df_generation.empty:
        ...
        df_generation['XM Type'] = df_generation['Values_code'].apply(lambda x : df_recursos.loc[df_recursos['Values_Code']==x,'Values_EnerSource'].values[0])
        
        #Add column with EMaps type from types dict
        df_generation['EM Type'] = df_generation['XM Type'].apply(lambda x: TYPES_XM_EM.get(x) if x in TYPES_XM_EM else 'unknown')
