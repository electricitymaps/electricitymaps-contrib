...
def fetch_production(zone_key='FO', session=None, target_datetime=None, logger=getLogger('FO')) -> dict:
    if target_datetime:
    ...
    for key, value in obj.items():
        if key == 'tiden':
            data['datetime'] = arrow.get(
                arrow.get(value).datetime, 'Atlantic/Faroe').datetime
        elif 'Sum' in key:
            continue
        elif 'Test' in key:
            continue
            ...
            data['production'][generation_type] = data['production'].get(generation_type, 0) + value
        else:
