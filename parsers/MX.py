...
def fetch_csv_for_date(dt, session=None):
    ...
    parameters = {
        ...
        "ctl00_ContentPlaceHolder1_FechaConsulta_ClientState": "{\"minDateStr\":\"" + datestr + "+0:0:0\",\"maxDateStr\":\"" + datestr + "+0:0:0\"}",
    ...
    data = urllib.parse.urlencode(parameters, quote_via=urllib.parse.quote).replace("%2B0", "+0")
    
    response = session.post(MX_PRODUCTION_URL, data=data, headers={"Content-Type": "application/x-www-form-urlencoded"})
    response.raise_for_status()
    
    # API returns normally status 200 but content type text/html when data is missing
    if "Content-Type" not in response.headers or response.headers["Content-Type"] != 'application/octet-stream':
        raise Exception("Error while fetching csv for date {}: No CSV was returned by the API. Probably the data for this date has not yet been published.".format(datestr))
    
    # skip non-csv data, the header starts with "Sistema"
    csv_str = response.text
    csv_str = csv_str[csv_str.find("\"Sistema\""):]
    
    return pd.read_csv(StringIO(csv_str), parse_dates={'instante' : [1, 2]}, date_parser=parse_date)


def convert_production(series):
    aggregated = {'biomass': 0.0,
          'coal': 0.0,
          'gas': 0.0,
          'hydro': 0.0,
          'nuclear': 0.0,
