...
def parse_from_entsoe_api(zone, token):
	...
	if zone not in ENTSOE_DOMAIN_MAPPINGS:
		print(f"Zone {zone} does not exist in the ENTSOE domain mapping", file=sys.stderr)
		exit(1)
	...
	url = f"https://transparency.entsoe.eu/api?securityToken={token}&documentType=A68&processType=A33&in_Domain={domain}&periodStart={date}0000&periodEnd={date}0000"
	...
	try:
		...
		raise ValueError(f"Data for zone {zone} could not be retrieved from ENTSOE", e)
