...
@objc open class CAPBridgeViewController: UIViewController {
  ...
  /**
      Loads and sets up the main view of the view controller, including initializing the web view and Capacitor bridge.

      This method performs the following steps:
      - Loads the instance configuration for the Capacitor environment.
      - Sets logging settings and logs any configuration warnings.
      - Sets default status bar and screen orientation settings.
      - Prepares the web view with appropriate configurations and delegates.
      - Creates a container view to hold the web view.
      - Adds the web view to the container view and sets up Auto Layout constraints to respect safe area insets, preventing UI elements from overlapping with system elements like the status bar.
      - Sets the container view as the main view of the view controller.
      - Initializes the Capacitor bridge with the configuration and delegates.
      - Calls `capacitorDidLoad()` to perform any additional setup after loading the bridge.
      - Updates the binary version information if necessary.

      - Note: This method overrides `loadView()` and is called when the view controller's view needs to be created. It sets up the web view inside a container view with Auto Layout constraints to ensure UI elements are properly positioned within the safe area.
  */
    ...
    override public final func loadView() {
      ...
      containerView.backgroundColor = webView?.backgroundColor ?? UIColor.systemBackground
    ...
    open func webViewConfiguration(for instanceConfiguration: InstanceConfiguration) -> WKWebViewConfiguration {
      ...
      // Add logging
      print("Configuring WebView with instance configuration: \(instanceConfiguration)")
