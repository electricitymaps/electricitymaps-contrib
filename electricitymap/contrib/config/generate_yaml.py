...
for exchange_key, exchange_config in exchanges.items():
    zone_keys = exchange_key.split("->")
    _exchange_key_unicode = "→".join(zone_keys)
    with open(
        CONFIG_DIR.joinpath(f"exchanges/{_exchange_key_unicode}.yaml"), "w", encoding="utf-8"
    ) as f:
...
for zone_key, zone_config in zones.items():
    ...
    if zone_key in CO2EQ_PARAMETERS_LIFECYCLE["emissionFactors"]["zoneOverrides"]:
        e_f = (
            _zone_config["emissionFactors"] if "emissionFactors" in _zone_config else {}
        )
        e_f["lifecycle"] = CO2EQ_PARAMETERS_LIFECYCLE["emissionFactors"][
            "zoneOverrides"
        ][zone_key]
